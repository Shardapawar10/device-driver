...
void out_char(struct timer_list *tm)
{
    ...
    ret = kfifo_out(&mybuf, &ch, 1);
    mod_timer(&timer, jiffies +  msecs_to_jiffies(1000));
    flag=1;
    if(ret == 1)
        pr_info("%s char from fifo: %c\n", THIS_MODULE->name, ch);
}
...
static int __init pchar_init(void)
{
    ...
    // create device class
    pclass = class_create("pchar_class111");
    if (IS_ERR(pclass))
    {
    ...
    printk(KERN_INFO "%s: class_create() created pchar_class111.\n", THIS_MODULE->name);
    // create device file
...
static void __exit pchar_exit(void)
{
    ...
    class_destroy(pclass);
    printk(KERN_INFO "%s: class_destroy() destroyed pchar_class111.\n", THIS_MODULE->name);
